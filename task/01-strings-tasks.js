...
function getRectangleString(width, height) {
    let p = ('│' + ' '.repeat(width - 2) + '│\n').repeat(height - 2);
    let s = "".concat(('┌' + '─'.repeat(width - 2) + '┐\n'),
             p, ('└' + '─'.repeat(width - 2) + '┘\n'));
    return s;
}
...
function encodeToRot13(str) {
    ...
    {
        if (alphabetCaps.indexOf(str[i]) != -1)
        {
            result += alphabetCaps[(alphabetCaps.indexOf(str[i]) + 13) % 26];
        }
        else if (alphabetSmall.indexOf(str[i]) != -1)
        {
            result += alphabetSmall[(alphabetSmall.indexOf(str[i]) + 13) % 26];
        }
        else 
        {
            result += str[i];
...
function getCardId(value) {
    ...
    return suits.indexOf(value[size - 1]) * ranks.length + ranks.indexOf(value.substr(0, size - 1));
}
