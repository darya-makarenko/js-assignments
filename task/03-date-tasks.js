...
function angleBetweenClockHands(date) {
    let degreeHour = 0.5 * (60 * (date.getUTCHours() % 12) + date.getUTCMinutes());
    let degreeMinute = 6 * date.getUTCMinutes();
    let diff = Math.abs(degreeMinute - degreeHour);
    if (diff > 180)
    { 
        diff = diff - 180;
    }
